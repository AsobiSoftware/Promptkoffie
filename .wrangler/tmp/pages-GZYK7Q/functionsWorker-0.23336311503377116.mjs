...
__name(emailExists, "emailExists");
async function countByType(env, type) {
  const rows = await getAllRows(env);
  return rows.slice(1).filter((row) => (row[2] || "").trim().toLowerCase() === type).length;
}
__name(countByType, "countByType");
...
3. E\xE9n herinnering \u2014 maar alleen als je 'm dan nog niet ge\xEFnstalleerd hebt
...
3. E\xE9n follow-up \u2014 maar alleen als we dan nog niks van je gehoord hebben

Daarna hoor je niks meer van ons, tenzij je zelf contact opneemt.`
};

// api/signup/business.ts
...
var onRequestPost2 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  try {
    ...
    return json2({ success: false, error: "Ongeldige aanvraag." }, 400);
  }
  const email = (body.email || "").trim().toLowerCase();
  if (!email || !email.includes("@") || !email.includes(".")) {
    return json2({ success: false, error: "Vul a.u.b. een geldig e-mailadres in." }, 400);
  }
  try {
    if (await emailExists(env, email)) {
      const totalUsers2 = await countByType(env, "user");
      return json2({
        success: true,
    ...
    return json2({
      success: true,
      message: "Hoera! Je bent toegevoegd aan de PromptKoffie b\xE8ta-wachtlijst.",
      totalUsers
    });
  } catch (err) {
    console.error("Fout bij user-inschrijving:", err);
    return json2({ success: false, error: "Er ging iets mis. Probeer het later opnieuw." }, 500);
  }
}, "onRequestPost");
...
// ../.wrangler/tmp/pages-GZYK7Q/functionsRoutes-0.6540013671363114.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-PAEwV4/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-PAEwV4/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.23336311503377116.mjs.map
