...
import { Sparkles, Coffee, ShieldCheck, ArrowUpRight, Zap, Paperclip, Send, Mic, Plus, Bot, User, Check, Sparkle } from 'lucide-react';

interface InteractiveSimulatorProps {
  embedded?: boolean;
}

export const InteractiveSimulator: React.FC<InteractiveSimulatorProps> = ({ embedded = false }) => {
      ...
      <div className="bg-[#1C1917] border border-[#332F2C] rounded-2xl shadow-2xl overflow-hidden text-left">
        ...
        <div className="px-4 py-2.5 bg-[#141210] border-b border-[#292524] flex items-center justify-between text-xs text-stone-400">
          <div className="flex items-center gap-3">
            ...
            <div className="px-3 py-1 bg-[#23201D] rounded-md border border-[#332F2C] text-[11px] font-mono text-stone-300 flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-2 text-[11px] font-medium text-amber-300 bg-amber-950/70 border border-amber-800/60 px-2.5 py-1 rounded-md">
        ...
        {platform === 'chatgpt' && (
          <div className="bg-[#212121] text-stone-200 min-h-[380px] flex flex-col sm:flex-row">
            {/* Sidebar Mockup */}
            ...
            <div className="flex-1 flex flex-col justify-between p-4 sm:p-6 bg-[#212121] relative">
              {/* Top Chat Header */}
              <div className="flex items-center justify-between border-b border-[#2f2f2f] pb-3 mb-4 text-xs font-semibold text-stone-300">
                ...
                <div className="flex items-start gap-3 justify-start">
                  ...
                  <div className="bg-[#262626] border border-[#333333] p-3.5 rounded-2xl rounded-tl-xs max-w-lg text-xs sm:text-sm text-stone-200 shadow-sm">
                    ...
                    <div className="flex items-center gap-1.5 py-1">
                      ...
                      <span className="text-stone-400 text-xs ml-2 italic">Onderzoeksrapport analyseren & kernpunten formuleren...</span>
                    </div>
                  </div>
                </div>
              </div>

              {/* ChatGPT Bottom Area: Ultra-thin Promptkoffie Extension Banner placed STRICTLY ABOVE input box */}
              <div className="relative pt-2">
                {/* === INJECTED PROMPTKOFFIE CHROME EXTENSION BANNER (PLACED ABOVE INPUT) === */}
                <div className="mb-2 bg-[#1A1918] border border-amber-500/40 rounded-xl px-3 py-1.5 shadow-md flex items-center justify-between gap-2 text-xs">
                  <div className="flex items-center gap-2.5 overflow-hidden">
                    {/* Promptkoffie / Brand Logo Badge */}
                    <div className="w-5 h-5 rounded bg-[#B45309] text-white flex items-center justify-center font-bold text-[10px] shrink-0">
                      ☕
                    </div>
                    {/* Brand Logo & Headline */}
                    <div className="flex items-center gap-2 truncate">
                      <span className="font-bold text-amber-400 text-xs shrink-0 flex items-center gap-1">
                        <span className="w-2 h-2 rounded-full bg-blue-500 inline-block" />
                        Swapfiets
                      </span>
                      <span className="text-[11px] text-stone-300 truncate">
                        — Altijd een werkende e-bike of stadsfiets voor je werk of studie. Vanaf €16,90/mnd.
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {/* ChatGPT Input Box */}
                <div className="bg-[#2f2f2f] border border-[#424242] rounded-2xl p-2.5 flex items-center justify-between gap-2 shadow-inner">
        ...
        {platform === 'gemini' && (
          <div className="bg-[#F8FAFC] text-slate-800 min-h-[380px] flex flex-col justify-between p-4 sm:p-6 relative">
            ...
            {/* Gemini Prompt Bar & Injected Extension Overlay */}
            <div className="relative pt-2">
              {/* === INJECTED PROMPTKOFFIE EXTENSION OVERLAY (ABOVE INPUT) === */}
              <div className="mb-2 bg-[#0F172A] text-white border border-amber-500/40 rounded-xl px-3 py-1.5 shadow-md flex items-center justify-between gap-2 text-xs">
                <div className="flex items-center gap-2.5 overflow-hidden">
                  <div className="w-5 h-5 rounded bg-[#B45309] text-white flex items-center justify-center font-bold text-[10px] shrink-0">
                    ☕
                  </div>
                  <div className="flex items-center gap-2 truncate">
                    ...
                    <span className="text-[11px] text-slate-300 truncate">
                      — Boeken, elektronica & kantoorbenodigdheden direct morgen in huis.
                    </span>
                  </div>
                </div>
              </div>
              ...
              {/* Gemini Input Box */}
              <div className="bg-slate-100 border border-slate-300 rounded-full px-4 py-2.5 flex items-center justify-between gap-2 shadow-inner">
        ...
        {platform === 'claude' && (
          <div className="bg-[#FAF6F0] text-[#2C2825] min-h-[380px] flex flex-col justify-between p-4 sm:p-6 relative">
            {/* Claude Top Header */}
            <div className="flex items-center justify-between border-b border-[#E8E0D5] pb-3 mb-4 text-xs font-serif">
              <div className="flex items-center gap-2">
            ...
            {/* Claude Input & Injected Extension Overlay */}
            <div className="relative pt-2">
              {/* === INJECTED PROMPTKOFFIE EXTENSION OVERLAY (ABOVE INPUT) === */}
              <div className="mb-2 bg-[#221F1C] text-white border border-amber-500/40 rounded-xl px-3 py-1.5 shadow-md flex items-center justify-between gap-2 text-xs">
                <div className="flex items-center gap-2.5 overflow-hidden">
                  <div className="w-5 h-5 rounded bg-[#B45309] text-white flex items-center justify-center font-bold text-[10px] shrink-0">
                    ☕
                  </div>
                  <div className="flex items-center gap-2 truncate">
                    ...
                    <span className="text-[11px] text-stone-300 truncate">
                      — Leer Spaans in 5 min/dag terwijl AI je antwoord genereert.
                    </span>
                  </div>
                </div>
              </div>
              ...
              {/* Claude Input Box */}
              <div className="bg-white border border-[#DCD0C0] rounded-xl p-3 flex items-center justify-between gap-2 shadow-xs">
        ...
        {/* Extension Footer Guarantee */}
        <div className="bg-[#141210] px-4 py-3 border-t border-[#292524] flex flex-wrap items-center justify-between gap-2 text-xs text-stone-400">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping" />
            <span className="font-semibold text-stone-200">Zo werkt de Chrome Extensie:</span>
            <span className="text-stone-400">Verschijnt alleen als AI antwoord typt. Verdwijnt automatisch.</span>
          </div>
